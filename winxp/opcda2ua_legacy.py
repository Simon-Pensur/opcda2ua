...
def parse_args():
    ...
    parser.add_argument(
        "--azucarlito",
        action="store_true",
        help="Filtro para SCADA azucarero. Tags con .medida: expone "
...
class LegacyBridge(object):
    ...
    def __init__(self, da_server, da_host="localhost", ua_port=4840,
        ...
        self.mode = mode  # None, 'azucarlito', or 'ifix'
    ...
    # Azucarlito: properties to keep for .velocidad objects
    ...
    def _deduplicate_tags(self, tags):
        ...
        WinCC exposes the same tags under multiple paths, e.g.:
          AZUCARLITO_SERVIDOR01::FAB_X.medida
    ...
    def _filter_tags(self, tags):
        ...
        Modes:
          ...
          'azucarlito' - keep .medida tags (solo value), .velocidad objects
        ...
        if self.mode == 'azucarlito':
            tags = self._filter_azucarlito(tags)
        elif self.mode == 'ifix':
    ...
    def _filter_azucarlito(self, tags):
        """Azucarlito mode: smart filtering for sugar mill SCADA.
        ...
        log.info("Azucarlito filter: %d medida (%d energy in %d groups), "
    ...
    def _read_descriptions(self, opc_client, tags):
        ...
        In azucarlito mode: reads X_I.descripcion for each base object X
        ...
        Returns dict {key: description_string} where key is:
          - full DA tag name (azucarlito mode)
    ...
    def _apply_descriptions(self, descs):
        ...
        In azucarlito mode: descriptions are keyed by DA tag name.
    ...
    def _parse_tag_group(self, tag):
        ...
        if dot > 0:
            base = tag[:dot]
            prop = tag[dot + 1:]
            if self.mode == 'azucarlito' and prop == 'medida':
        ...
        # Standalone tag (no dot)
        if self.mode == 'azucarlito' and tag in self._energy_bases:
    ...
    def _add_ua_node(self, tag, value):
        ...
        In azucarlito/ifix mode, tags sharing a common prefix before '.'
...
def main():
    ...
    if args.azucarlito:
        mode = 'azucarlito'
        log.info("Mode: azucarlito (sugar mill SCADA filtering)")
    elif args.ifix:
        mode = 'ifix'
